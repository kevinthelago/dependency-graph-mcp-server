    ...
    for (const useDecl of uses) {
      for (const flat of useDecl.flat) {
        ...
        if (resolved.targetFile != null) {
          ...
          imports.push({ specifier: flat.segments.join('::'), resolvedPath: resolved.targetFile, isExternal: false, isUnresolved: false, wildcard: flat.wildcard })
          ...
          imports.push({ specifier: resolved.externalSpec, isExternal: true, isUnresolved: false, wildcard: flat.wildcard })
        } else {
          ...
          imports.push({ specifier: flat.segments.join('::'), isExternal: false, isUnresolved: true, wildcard: flat.wildcard })
        }
      }
    }
