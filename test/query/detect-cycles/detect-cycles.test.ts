...
import { createGraph, createGraphView } from "../../../src/graph/store.js";
...
function mkCtx(
  ...
  for (const n of nodes) {
    g.addNode(n.id, {
      id: n.id,
      kind: n.kind,
      language: n.kind === "file" ? "ts" : null,
      name: n.name,
      ...(n.file !== undefined ? { file: n.file } : {}),
    });
  }
  for (const e of edges) {
    // Self-loops use addDirectedEdge with same source+target
...
describe("detect_cycles – module vs symbol granularity", () => {
  ...
  it("module mode excludes external nodes", () => {
    ...
    const g = createGraph();
    g.addNode(A, { id: A, kind: "file", language: "ts", name: "src/a.ts" });
    g.addNode(EXT, { id: EXT, kind: "external", language: null, name: "lodash" });
    // Even if A imports and re-exports ext (which is impossible in practice),
    // external nodes must be filtered out
    g.addDirectedEdge(A, EXT, { from: A, to: EXT, kind: "import", targetType: "external", resolution: "resolved" });
    g.addDirectedEdge(EXT, A, { from: EXT, to: A, kind: "import", targetType: "file", resolution: "resolved" });
...
describe("detect_cycles – truncation", () => {
  ...
  it("group truncated=true when SCC size > maxGroupSize", () => {
    ...
    const g = createGraph();
    for (const id of ids) {
      g.addNode(id, { id, kind: "file", language: "ts", name: id.slice(5) });
    }
    // Chain them into a single cycle
    for (let i = 0; i < ids.length; i++) {
      const from = ids[i]!;
      const to = ids[(i + 1) % ids.length]!;
      g.addDirectedEdge(from, to, { from, to, kind: "import", targetType: "file", resolution: "resolved" });
    }
